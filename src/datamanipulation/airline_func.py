...
def Airline_Func(data):
    ...
    data['DEPARTURE_DELAY_NORMALISED'] = preprocessing.MinMaxScaler().fit_transform(
        data[['DEPARTURE_DELAY']].values.astype(float))
    # binarise the column "ORIGIN_AIRPORT"
    data = data.merge(pd.get_dummies(data['ORIGIN_AIRPORT']))
    return(data,dfs,data_AA,Data_Small_Delay)
